...
internal class EstablishmentUpdateOnlyRepository(NowasteDbContext dbContext)
    : IEstablishmentUpdateOnlyRepository
{
    private readonly NowasteDbContext _dbContext = dbContext;

    public async Task<EstablishmentEntity?> GetById(Guid Id)
    {
        return await _dbContext.Establishments.FirstOrDefaultAsync(establishment =>
            establishment.Id == Id
        );
    }
    ...
    {
        return await _dbContext.OperatingDays.FirstOrDefaultAsync(operatingDay =>
