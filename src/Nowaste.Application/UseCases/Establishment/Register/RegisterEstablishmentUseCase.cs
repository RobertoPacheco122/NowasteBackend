...
) : IRegisterEstablishmentUseCase
{
    ...
    public async Task<ResponseRegisteredEstablishmentJson> Execute(
        RequestRegisterEstablishmentJson request
    )
    {
        ...
        var userEntity =
            await _userUpdateOnlyRepository.GetById(loggedUser.Id)
    ...
    private async Task Validate(RequestRegisterEstablishmentJson request)
    {
        ...
        var cnpjOrEmailAlreadyExist =
            await _establishmentReadOnlyRepository.ExistActiveEstablishmentWithCnpjOrEmail(
                request.Email,
                request.Cnpj
            );

        if (cnpjOrEmailAlreadyExist)
            establishmentValidationResult.Errors.Add(
        ...
        if (
            establishmentValidationResult.IsValid is false
            || addressValidationResult.IsValid is false
        )
        {
            var establishmentErrorsMessages = establishmentValidationResult
                .Errors.Select(f => f.ErrorMessage)
                .ToList();
            var addressErrorsMessages = addressValidationResult
                .Errors.Select(f => f.ErrorMessage)
                .ToList();
