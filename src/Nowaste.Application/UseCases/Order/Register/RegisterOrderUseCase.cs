...
public class RegisterOrderUseCase(
    ...
    public async Task<ResponseRegisteredOrderJson> Execute(RequestRegisterOrderJson request) {
        ...
        var establishmentEntity = await _establishmentReadOnlyRepository
            .GetById(request.EstablishmentId) ??
                throw new NotFoundException("Estabelecimento não encontrado.");

        var addressEntity = await _addressReadOnlyRepository.GetById(request.AddressId) ??
            throw new NotFoundException("Endereço não encontrado.");

        var establishmentOrdersCount = await _orderReadOnlyRepository
            .GetOrdersCountByEstablishment(request.EstablishmentId);
        ...
        var allPurchasedProductsEntities = await _productReadOnlyRepository
            .GetAllByIds(allPurchasedProductsIds);
        ...
        var fullDeliveryAddress = $"{addressEntity.StreetName}, {addressEntity.Number} - {addressEntity.Neighborhood}" +
