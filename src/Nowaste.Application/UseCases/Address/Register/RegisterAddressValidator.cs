...
public partial class RegisterAddressValidator : AbstractValidator<RequestRegisterAddressJson> {
    public RegisterAddressValidator() {
        ...
        RuleFor(address => address.State)
            .NotEmpty()
            .WithMessage("O estado é obrigatória.");
        ...
        RuleFor(x => x)
            ...
            .WithMessage("Não é permitido preencher mais de dois dos seguintes campos simultaneamente: PersonId, EstablishmentId, InstitutionId.");

        RuleFor(x => x)
           .Must(HaveAtLeastOneIdFilled)
           .WithMessage("Pelo menos um dos seguintes campos deve ser preenchido: PersonId, EstablishmentId, ou InstitutionId.");
    }
