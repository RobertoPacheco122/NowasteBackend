...
public class UpdateAddressByPersonUseCase(
    ...
    public async Task Execute(Guid addressId, RequestRegisterAddressJson request) {
        ...
        var addressEntity = await _addressUpdateOnlyRepository.GetById(addressId) ??
        ...
        var previousWorkAddress = allEstablishmentAddresses
            .FirstOrDefault(a => a.AddressType == Domain.Enums.EAddressType.Operational);

        var previousHomeAddress = allEstablishmentAddresses
            .FirstOrDefault(a => a.AddressType == Domain.Enums.EAddressType.Home);

        if (request.AddressType == EAddressType.Work &&
            previousWorkAddress is not null) {
            previousWorkAddress.AddressType = Domain.Enums.EAddressType.Common;

            _addressUpdateOnlyRepository.Update(previousWorkAddress);
        }

        if (request.AddressType == EAddressType.Home &&
            previousHomeAddress is not null) {
