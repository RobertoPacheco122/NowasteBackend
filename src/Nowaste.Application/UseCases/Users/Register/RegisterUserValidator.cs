...
public class RegisterUserValidator : AbstractValidator<RequestRegisterUserJson>
{
    ...
    public RegisterUserValidator()
    {
        RuleFor(user => user.Email)
            ...
            .EmailAddress()
            .When(
                user => string.IsNullOrWhiteSpace(user.Email) is false,
        ...
        RuleFor(user => user)
            ...
            .WithMessage(
                "O usuário não pode estar associado a um estabelecimento e instituição ao mesmo tempo."
            );

        RuleFor(user => user.Role).NotEmpty().WithMessage("A role é obrigatória.");
        ...
        When(
            user => (user.InstitutionId.HasValue && user.EstablishmentId.HasValue) is false,
            () =>
            {
                RuleFor(user => user.Role)
                    ...
                    .WithMessage(
                        "A role deve ser do tipo 'institution' quando InstitutionId estiver preenchido."
                    );
                ...
                RuleFor(user => user.Role)
                    ...
                    .WithMessage(
                        "A role deve ser do tipo 'establishment' quando EstablishmentId estiver preenchido."
                    );
            }
        );

        RuleFor(user => user.FullName).NotEmpty().WithMessage("O nome completo é obrigatório.");
        ...
        RuleFor(user => user.PhoneNumber)
            ...
            .MinimumLength(10)
            .When(
                user => string.IsNullOrWhiteSpace(user.PhoneNumber) is false,
                ApplyConditionTo.CurrentValidator
            )
